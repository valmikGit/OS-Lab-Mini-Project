...
void *handle_client(void *arg) {
    ...
    if (is_admin) {
        ...
        while (1) {
            ...
            // Check if the client wants to perform another action
            send(client_socket, "Do you want to perform another action? (yes/no): ", strlen("Do you want to perform another action? (yes/no): "), 0);
            bytes_read = read(client_socket, buffer, BUF_SIZE - 1);
            buffer[bytes_read] = '\0'; // Ensure null-terminated string
            printf("Admin's choice to perform another action: %s\n", buffer); // Debug print
...
// Display books for non-admin users
        send(client_socket, "Library Books:\n", strlen("Library Books:\n"), 0);
        display_books(client_socket);
    }
